...
4. `quick_edit`가 선택 확정되면, Beads-enabled repo에서는 **새 standalone issue를 생성**하고 그 **새 issue에 `quick_edit` 라벨**을 붙인다.
...
6. `writing-plans` 수정이나 parent linkage 의무가 요구되지 않는지 확인한다.
