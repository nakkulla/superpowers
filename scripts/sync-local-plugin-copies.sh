...
usage() {
  cat <<'USAGE'
Usage: scripts/sync-local-plugin-copies.sh copy | verify

  copy    Sync tracked working-tree files to installed Claude plugin copies
  verify  Verify installed Claude plugin copies match tracked source files
...
ensure_repo_root() {
  if ! git -C "$REPO_ROOT" rev-parse --show-toplevel >/dev/null 2>&1; then
    echo "error: SUPERPOWERS_REPO_ROOT is not a git repository: $REPO_ROOT" >&2
    exit 1
  fi
}
...
cmd_verify() {
  ...
  echo ""
  echo "All checked Claude plugin copies are in sync."
}
